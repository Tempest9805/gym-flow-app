...
import { useWeekSchedule, useCurrentProfile } from '@/lib/hooks';
...
export function UserHome({ profile }: UserHomeProps) {
  ...
  // Usar el perfil proporcionado o el más reciente
  const currentProfile = profileData || profile;
  const { data: schedule } = useWeekSchedule(currentProfile?.id);
    ...
    <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
      <View className="mb-6">
        <Text className="text-3xl font-bold text-text-primary">
          Hello, {currentProfile?.full_name || currentProfile?.email?.split('@')[0] || 'Athlete'}
        </Text>
        <Text className="text-lg text-text-secondary mt-1">Ready for today&apos;s session?</Text>
      </View>

      {/* Weekly Agenda - solo si hay perfil */}
      {currentProfile?.id && (
        <View className="mb-8">
          <Text className="text-sm font-bold text-text-secondary uppercase mb-3">This Week</Text>
          <WeeklyAgenda profile={currentProfile} />
        </View>
      )}

      {/* Current Routine Card */}
      {todaysRoutine ? (
        <CardBase className="mb-8 p-6 bg-surface-secondary border-border">
          <Text className="text-sm font-bold text-primary uppercase mb-2">Today's Routine</Text>
